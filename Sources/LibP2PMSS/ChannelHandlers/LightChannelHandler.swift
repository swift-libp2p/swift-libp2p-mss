...
internal final class LightMultistreamSelectHandler:ChannelInboundHandler, RemovableChannelHandler {
    ...
    private enum MessageEvent:EventType, Hashable {
        static func == (lhs: LightMultistreamSelectHandler.MessageEvent, rhs: LightMultistreamSelectHandler.MessageEvent) -> Bool {
    ...
    init(mode:LibP2P.Mode, protocols:[String], logger:Logger, upgradePromise:EventLoopPromise<(protocol:String, leftoverBytes:ByteBuffer?)>, uuid:String) {
        self.uuid = uuid
        self.logger = logger
        self.mode = mode
    ...
    private func handleNegotiationMessage(_ buffer:ByteBuffer, context:ChannelHandlerContext) -> MSSState? {
        ...
        self.logger.trace("State[\(self.state.state)] -> `\(String(data: Data(buffer.readableBytesView), encoding: .utf8) ?? "")`")
        
        //Feed the inbound message into our MSS Negotiator
        switch protocolNegotiator.consumeMessage( Array<UInt8>(buffer.readableBytesView) ) {
        case .stillNegotiating(let response):
            ...
            if let res = response {
                self.logger.trace("Still Negotiating, Responding with: `\(String(data: Data(res), encoding: .utf8) ?? "")`")
        ...
        case .negotiated(let negotiatedProtocol, let response, let leftoverBytes):
            ...
            logger.trace("We aggreed on the protocol `\(negotiatedProto)`, echoing back proto and installing the appropriate handlers")
